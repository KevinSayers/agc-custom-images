...
class AwsBatchExecutor(ClusterExecutor):
    def __init__(self, workflow, dag, cores,
        ...
        exec_job = exec_job or (
            ...
            "--notemp --no-hooks --nolock " % self.get_snakefile(workflow)
        )
    ...
    def get_snakefile(self, workflow):
        """The snakefile must be a relative path, which should be derived
        from the self.workflow.main_snakefile.
        """
        assert os.path.exists(workflow.main_snakefile)
        self.workdir = os.path.dirname(workflow.persistence.path)
        return workflow.main_snakefile.replace(self.workdir, "").strip(
            os.sep
        )
    ...
    def _setup_job_def(self):
        ...
        # TODO: Use second image and create script to execute (required for PR)
        if self.assume_shared_fs is False:
            container_properties["image"] = "680431765560.dkr.ecr.us-west-2.amazonaws.com/aws/snakemake-mirror:internal-fork"
        else:
            container_properties["image"] = "snakemake/snakemake:v6.15.5"
    ...
    def _generate_snakemake_command(self, job):
        ...
        if self.assume_shared_fs is False:
            # TODO: Modify so that it isn't referenced directly from AGC
            command[0] = "s3://{}/workflow.zip".format(self.snakemake_data)
        else:
            return_command = ["sh", "-c"]
